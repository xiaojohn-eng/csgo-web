async(page)=>{
  ...
  const frames=[];
  for(const mode of ['r2','r3']){
    await page.evaluate(value=>window.__CSGO_ASSET_PREVIEW__.enablePropLighting(true,{enablePlainUnbumped:true,enableDecalMultiply:value==='r3'}),mode);
    for(const team of ['ct','t']){
      ...
      const screenshot=`output/playwright/source-vhv-r3-${team}-${mode}.png`;
  ...
  const evidence={session:'csgo-vhv-r3',headless:true,url:page.url(),viewport:page.viewportSize(),
  ...
  await page.evaluate(value=>window.__CSGO_VHV_R3_EVIDENCE__=value,evidence);
  ...
  for(const team of ['ct','t']){
    ...
    const [r2,r3]=teamFrames;
    if(r2.state.lighting?.appliedMeshes!==1968||r3.state.lighting?.appliedMeshes!==2008||r3.state.lighting?.decalMultiplyMeshes!==40)
      throw Error('Unexpected VHV branch coverage');
    for(const next of teamFrames.slice(1))for(const key of ['calls','triangles'])if(r2.state.asset.render[key]!==next.state.asset.render[key])
