"""Actual original AWP normal spread branches and command seed scope.
...
s=importlib.util.spec_from_file_location('awp','%s/scripts/probe-source-awp-handling.py'%ROOT);p=importlib.util.module_from_spec(s);s.loader.exec_module(p)
d=p.d;a=p.a;f=p.f;SCHEMA=d.T+0xa000
class AWPSpread(p.AWPAccuracy):
 def __init__(self):
...
def main():
 ps,sha=p.profiles();engine=AWPSpread();rows=[];command_seeds=[]
 for weapon in ['awp']:
  attrs=ps[weapon]
  for mode in [0,1]:
 ...
 # Original command scope installs both globals; repeated primary calls consume the
 ...
 for command_seed,server_seed in [(0,0),(42,256),(5426,0x12345678),(4484,255),(0x7fffffff,0x7fffffff),(-1,-1),(-0x80000000,-0x80000000),(123456,987654)]:
  ...
  for at,stop in [(0xd4b494,0xd4b49b)]*3:
 ...
 report=dict(status='original_awp_item9_spread_and_seed_executed',sourceServerSha256=d.SHA,sourceItemsSha256=sha,sourceVstdlibSha256=p.r.s.VSHA,
  functions=dict(seed=['0xcb539f','0xcb53ae'],commandSeedScope='0x600360',primarySeed='0xd4b494',spread=['0xcb5a30','0xcb57ed']),rows=rows,commandSeeds=command_seeds,
  limits=['Actual item identity mapped to distinct objects from R8 and Negev; original tests run.','External sincosf uses host sin/cos rounded f32; original RNG and offset SSE execute.'])
 (ROOT/'output/tests/source-awp-spread-native.json').write_text(json.dumps(report,separators=(',',':'))+'\n');print(json.dumps(dict(status=report['status'],spreadCases=len(rows),commandSeeds=len(command_seeds))))
