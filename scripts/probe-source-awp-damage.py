"""Execute the original normal bullet path with frozen awp item attributes.
...
spec = importlib.util.spec_from_file_location('original_awp_damage_blocks', ROOT / 'scripts/probe-source-damage.py')
native = importlib.util.module_from_spec(spec)
spec.loader.exec_module(native)
inventory = json.loads((ROOT / '.reference-assets/source-exports/awp-candidates/inventory.json').read_text())
...
rows, profiles = [], {}
for weapon, item, expected in [('awp','9',{'damage':115,'headshot multiplier':4,'armor ratio':1.95,'range':8192,'range modifier':.99})]:
    ...
    for distance in [0, 1, 100, 500, 1000, 2000, 4096, 8192]:
...
report = {'sourceServerSha256': native.SHA, 'sourceItems': source, 'profiles': profiles,
          'status': 'original_normal_awp_damage_blocks_executed', 'rows': rows,
...
(ROOT / 'output/tests/source-awp-damage-native.json').write_text(json.dumps(report, indent=2) + '\n')
