"""App740 pistol handling oracle: actual i386 consumers, isolated Unicorn.
...
def profiles():
 ...
 for weapon,name in [('glock18','weapon_glock_prefab'),('usp-s','weapon_usp_silencer_prefab')]:
  attrs={}
  def merge(key):
  ...
  merge(name);assert attrs['is full auto']=='0';out[weapon]=attrs
...
def main():
 ...
 report=dict(status='original_pistol_accuracy_recoil_punch_executed',sourceServerSha256=d.SHA,sourceItemsSha256=items_sha,sourceVstdlibSha256=r.s.VSHA,
  ...
  limits=['Accuracy/accepted-state consumers, not command acceptance/burst queue.', 'Both native numeric modes tested; mode ownership remains caller command state.', 'Semi-auto recoil uses prediction command seed &63, not recoilIndex or server bullet seed.', 'Ordinary default ConVars; ExoJump/turning modifiers excluded.', 'External expf/sincosf use host libm rounded f32; no complete glibc ulp claim.'])
 (ROOT/'output/tests/source-pistol-handling-native.json').write_text(json.dumps(report,separators=(',',':'))+'\n')
