"""Refresh the frozen reference hashes after the throw-variant snapshot merge.

scripts/patch-source-throw-snapshots.py appended the pin-pull preparation
graph (Upper_GREN over Aim_GREN + HandPos_GREN auto-layers) and the
Shoot_GREN2/Shoot_GREN3 release descriptors to every research snapshot body
dataset (existing descriptors keep their offsets, so every independently
verified sample is unchanged). The frozen provenance hashes must now point at
the patched files:
...
- the M4 actor-reference.json poseDataSHA256 (mirrors the production manifest
  entry for pose-data.json after scripts/merge-source-throw-variants.py).
"""
...
print('THROW_REFERENCE_REFRESH_DONE')
