"""App740 AWP handling oracle, reusing verified base consumers: actual i386 consumers, isolated Unicorn.
Command acceptance and FOV ordering have their own source-awp-command oracle.
"""
...
a=load('awp_accuracy_base','scripts/probe-source-accuracy.py');r=load('awp_recoil_base','scripts/probe-source-recoil.py');d=a.d;f=d.f32
...
def profiles():
 ...
 for weapon,name in [('awp','weapon_awp_prefab')]:
  attrs={}
  def merge(key):
...
class AWPAccuracy(a.NativeAccuracy):
 ...
 def setup(self,attrs,state,context,time=0,dt=1/60):
  ...
  self.ints(a.VT+0x688,[0xd47710]);self.ints(a.VT+0x6c0,[0xd4e890]);self.u.mem_write(d.T+0xec,b'\0');self.ints(d.T+0xc8,[5])
  self.floats(d.W+0xa8c,[context.get('zoomSmoothing',0),context.get('zoomReadyTime',0)]);self.ints(d.W+0xa6c,[context.get('mode',0)]);self.ints(0x16d2424,[context.get('commandSeed',0)])
...
def main():
 ps,items_sha=profiles();engine=AWPAccuracy();generator=AWPRecoil();tables={}
 ...
 rows=[];shots=[];deploy=[];sequences=[];reloads=[]
 for weapon,attrs in ps.items():
  for mode in [0,1]:
   ...
   # Original ladder branch, velocity clamps, zoom fields and exact decay boundary.
   for context in [dict(mode=mode,grounded=True,ladder=True,velocitySource=[0,0,80]),
     *[dict(mode=mode,grounded=False,velocitySource=[0,0,vz])for vz in [-1000,-20,0,20,1000]],
     *[dict(mode=mode,grounded=True,zoomReadyTime=ready,zoomSmoothing=.25)for ready in [0,1.05,1.15,10]]]:
    before=dict(neutral,penalty=.37,recoilIndex=3,lastShotTime=.1)
    rows.append(dict(weapon=weapon,mode=mode,context=context,before=before,time=1.1,dt=1/64,original=engine.tick(attrs,before,context,1.1,1/64)))
   for time in [f(f(1/64)+f(1.455))-1e-6,f(f(1/64)+f(1.455)),f(f(1/64)+f(1.455))+1e-6]:
    context=dict(mode=mode,grounded=True);before=dict(neutral,penalty=.1,recoilIndex=5)
    rows.append(dict(weapon=weapon,mode=mode,context=context,before=before,time=time,dt=1/64,original=engine.tick(attrs,before,context,time,1/64)))
 ...
 report=dict(status='original_awp_accuracy_recoil_punch_executed',sourceServerSha256=d.SHA,sourceItemsSha256=items_sha,sourceVstdlibSha256=r.s.VSHA,
  profiles=ps,tables=tables,rows=rows,shots=shots,deploy=deploy,sequences=sequences,reloads=reloads,defaults=engine.defaults,
  ...
  limits=['Accuracy/accepted-state consumers, not command acceptance.', 'Original AWP mode0/1, independently verified own vtable and attributes.', 'Semi-auto recoil uses prediction command seed &63, not recoilIndex or server bullet seed.', 'Ordinary default ConVars; ExoJump/turning modifiers excluded.', 'External expf/sincosf use host libm rounded f32; no complete glibc ulp claim.'])
 (ROOT/'output/tests/source-awp-handling-native.json').write_text(json.dumps(report,separators=(',',':'))+'\n')
