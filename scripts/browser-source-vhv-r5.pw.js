async(page)=>{
  ...
  const numeric=await page.evaluate(async()=>{
    const module=await import('/assets/source-exports/dust2-vhv/tint-decal/preview-source-prop-tint-decal.js');
    const oracle=await fetch('/assets/source-exports/dust2-vhv/encoding/tint-decal/evidence.json',{cache:'no-cache'}).then(r=>r.json());
    return module.runSourcePropTintDecalProbe(oracle.tokenFloat32Oracle);
  });
  ...
  const frames=[];
  for(const mode of ['r4','r5']){
    await page.evaluate(value=>window.__CSGO_ASSET_PREVIEW__.enablePropLighting(true,{enablePlainUnbumped:true,enableDecalMultiply:true,enableTintMask:true,enableTintDecal:value==='r5'}),mode);
    for(const team of ['ct','t']){
      ...
      const screenshot=`output/playwright/source-vhv-r5-${team}-${mode}.png`;
  ...
  const evidence={session:'csgo-vhv-r5',headless:true,url:page.url(),viewport:page.viewportSize(),
    numeric,camera:'original first CT/T spawn, eye64 source units, fov75; same spawnView API for every mode',baseline,released,frames,errors};
  await page.evaluate(value=>window.__CSGO_VHV_R5_EVIDENCE__=value,evidence);
  ...
  for(const team of ['ct','t']){
    ...
    const [r4,r5]=teamFrames;
    if(r4.state.lighting?.appliedMeshes!==2257||r5.state.lighting?.tintDecalMeshes!==68||r5.state.lighting?.decalMultiplyMeshes!==40||r5.state.lighting?.tintMaskMeshes!==249||r5.state.lighting?.appliedMeshes!==2325)
      throw Error('Unexpected VHV branch coverage');
    for(const next of teamFrames.slice(1))for(const key of ['calls','triangles'])if(r4.state.asset.render[key]!==next.state.asset.render[key])
