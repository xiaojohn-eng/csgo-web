"""Read-only original PCF closure export for the deployed smoke grenade.
...
Run: /Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup \
     --python-exit-code 1 --python scripts/export-source-grenade-smoke.py
"""
from pathlib import Path
import importlib.util, io, json, struct, sys

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / 'output/fidelity-character/grenade-smoke'
...
# CSmokeGrenadeProjectile's shipped client string and explosions_fx.pcf agree
# on this dispatcher. Named fallback is exported as a replacement, not emitted.
ROOTS = ['explosion_smokegrenade']
...
# must consume the resource exactly. The original vista smoke sheet has 34 sequences and 256 frames; all rectangles
# and durations are checked, rather than inferred from a name or padded.
...
pcf = 'particles/explosions_fx.pcf'
...
for name in ROOTS:
    if name not in systems:
        raise ValueError('Original smoke grenade system absent: ' + name)
...
for name in ROOTS:
    visit(systems[name])
pcf_file = write('original/explosions_fx.pcf', raw)
...
for source_value in raw_materials:
    ...
    for key, value in items.walk(definition):
        if isinstance(value, str) and ('texture' in str(key).lower() or str(key[-1]).lower() in ['$bumpmap', '$detail', '$normalmap']):
...
receipt = dict(format='source-grenade-smoke-particles-receipt-v1', context=context, roots=ROOTS, graph=graph_file,
...
print('GRENADE_SMOKE_PARTICLE_EXPORT', json.dumps(receipt, ensure_ascii=False))
