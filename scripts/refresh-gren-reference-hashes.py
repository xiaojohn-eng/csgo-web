"""Refresh the frozen reference hashes after the Shoot_GREN1 snapshot merge.

scripts/patch-source-gren-snapshots.py re-appended the five original
Shoot_GREN1 FRAMEANIM descriptors to every research snapshot body dataset in
the production variant order (existing descriptors keep their offsets, so
every independently verified sample is unchanged). The frozen provenance
hashes must now point at the patched files:
...
- the M4 actor-reference.json poseDataSHA256 (mirrors the production manifest
  entry for pose-data.json after scripts/merge-source-gren-poses.py).
"""
