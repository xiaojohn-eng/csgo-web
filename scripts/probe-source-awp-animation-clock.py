"""Reuse the original native clock harness with exact AWP MDL/event data."""
...
class NativeAWPClock(c.NativeClock):
    def __init__(self):
        super().__init__()
        audit=json.loads((ROOT/'.reference-assets/source-exports/awp-candidates/awp-ct/audit.json').read_text())
        entries,_=c.a.vpk.directory_index(c.a.vpk.GAME/'pak01_dir.vpk')
        entry=entries['models/weapons/v_snip_awp.mdl']
        ...
        assert self.mdlSha==audit['dependencies']['models/weapons/v_snip_awp.mdl']['sha256']
        ...
        for name in {e['name']for s in audit['sequences']for e in s['events']if e['name']}:
            if name.encode()+b'\0'not in self.raw:
                assert name=='AE_WPN_UNZOOM'
                self.registrations[name]=dict(id=0,type=1024,registrationVA=None,unregisteredServer=True)
                continue
...
def main():
    ...
    out=ROOT/'output/tests/source-awp-animation-clock-native.json'
    ...
    (ROOT/'game/source-awp-animation-data.ts').write_text('/** Generated from original AWP MDL and original ELF event registration. */\nexport const SOURCE_AWP_ANIMATION_DATA='+json.dumps(data,indent=2)+' as const;\n')
