"""Source-anchored private CT AK pose sampler, called by import-source-character.py.
...
def sample_combat(primary,parsed,buffers,cm,report,out):
    ...
    source='models/player/ct_animations.mdl'; mdl=parsed[source]; buffer=buffers[source]
    ...
    report['animationBoneMapping']={'sharedByName':len(shared),'mainOnly':sorted(primary_names.keys()-shared),
        ...
        'strategy':'Decode the original 70 CT animation bones; map 69 shared names, preserve two weapon helpers and three beret procedural bones at local rest. Beret jiggle dynamics are not implemented.'}
