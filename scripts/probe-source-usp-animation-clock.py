"""Reuse the original native clock harness with exact USP MDL/event data."""
...
class NativeUSPClock(c.NativeClock):
    def __init__(self):
        super().__init__()
        audit=json.loads((ROOT/'.reference-assets/source-exports/pistol-candidates/usp-ct/audit.json').read_text())
        entries,_=c.a.vpk.directory_index(c.a.vpk.GAME/'pak01_dir.vpk')
        entry=entries['models/weapons/v_pist_223.mdl']
        ...
        assert self.mdlSha==audit['dependencies']['models/weapons/v_pist_223.mdl']['sha256']
...
def main():
    ...
    out=ROOT/'output/tests/source-usp-animation-clock-native.json'
    ...
    (ROOT/'game/source-usp-animation-data.ts').write_text('/** Generated from original USP MDL and original ELF event registration. */\nexport const SOURCE_USP_ANIMATION_DATA='+json.dumps(data,indent=2)+' as const;\n')
