"""Decode original death sequences (Death1 + 8 deathpose_*, T + CT) through the audited FRAMEANIM chain.

Reuses the exact decoder pattern from scripts/export-source-jump-sequences.py
(StudioAnimDesc._read_frame_animations monkey-patch). Outputs per-team npz
frames, sequence metadata, and an audit report, plus a root-motion summary so
the death playback policy can be chosen from evidence.
"""
...
OUT_BASE = ROOT / '.reference-assets/source-exports/character-death'
...
SEQUENCES = {
    922: 'Death1', 923: 'deathpose_front', 924: 'deathpose_back', 925: 'deathpose_right',
    926: 'deathpose_left', 927: 'deathpose_crouch_front', 928: 'deathpose_crouch_back',
    929: 'deathpose_crouch_right', 930: 'deathpose_crouch_left',
}
...
def main():
    ...
    for team, source in TEAMS.items():
        ...
        descriptors = []; raw_frames = {}; sequence_records = []; root_motion = []
        try:
            for seq_index, expected in SEQUENCES.items():
                ...
                assert len(fields['boneWeights']) == len(mdl.bones) and all(w == 1.0 for w in fields['boneWeights'])
                pose_parameters = []
                ...
                sequence_records.append({'sourceModel': source, 'index': seq_index, 'name': seq.name,
                    'activityName': seq.activity_name, 'flags': int(seq.flags), 'category': 'death',
                ...
                rp = positions[:, root_index]
                root_motion.append({'sequence': seq.name, 'frames': desc.frame_count, 'fps': desc.fps,
                    'rootStart': rp[0].tolist(), 'rootEnd': rp[-1].tolist(),
                    'rootDelta': (rp[-1] - rp[0]).tolist(),
                    'frameToFrameRootMax': float(np.max(np.abs(np.diff(rp, axis=0)))),
                    'maxFrameToFrameBoneMotion': float(np.max(np.abs(np.diff(positions, axis=0))))})
        finally:
            StudioAnimDesc._read_frame_animations = patch

        # corner equivalence for [1,1] sequences: calc() at any cycle must equal frame()
        ...
        pose_parameters = sequence_records[0] and None or None
        pp = []
        for i in range(mdl.header.local_pose_paramater_count):
            start = mdl.header.local_pose_parameter_offset + i * 20
            with buffer.read_from_offset(start):
                name_offset, flags, low, high, loop = buffer.read_fmt('iifff')
            with buffer.read_from_offset(start + name_offset): name = buffer.read_ascii_string()
            pp.append({'index': i, 'name': name, 'flags': int(flags), 'start': low, 'end': high, 'loop': loop})
        metadata = {'sourceModel': source, 'boneNames': [b.name for b in mdl.bones],
            'poseParameters': pp, 'sequences': sequence_records, 'descriptors': descriptors}
        engine = module.PoseSampler(sampler_mdl, metadata, raw_frames)
        frame_err = rot_err = 0.0; checks = 0
        for record in sequence_records:
            key = record['animationIndices'][0]
            for cycle in (0.0, 0.37, 0.72, 1.0):
                expected = engine.frame(key, cycle)
                for mode in ('sdk-3way', 'sdk-bilinear'):
                    actual = engine.calc(record, cycle, {}, mode)
                    frame_err = max(frame_err, float(np.max(np.abs(actual[0] - expected[0]))))
                    rot_err = max(rot_err, float(np.max(1 - np.abs(np.sum(actual[1] * expected[1], axis=1)))))
                    checks += 1
        assert frame_err < 1e-10 and rot_err < 1e-10, (frame_err, rot_err)
        ...
        npz_path = out / 'death-frames.npz'
        np.savez_compressed(npz_path, **packed)
        npz_bytes = npz_path.read_bytes()
        metadata['frameFile'] = 'death-frames.npz'
        meta_bytes = (json.dumps(metadata, indent=1) + '\n').encode()
        (out / 'death-metadata.json').write_bytes(meta_bytes)
        audit = {'status': 'passed_death_frame_decode', 'team': team, 'sourceModel': source,
            'sourceMdlSha256': sha(raw_bytes), 'sourceMdlBytes': len(raw_bytes),
            'aniResolved': True, 'aniSha256': sha(ani.buffer.read() if hasattr(ani.buffer, 'read') else bytes(ani.buffer)),
            'sequences': [{'index': r['index'], 'name': r['name'], 'activityName': r['activityName'],
            ...
            'samplerEquivalence': {'checks': checks, 'maxPositionError': frame_err, 'maxQuaternionDotError': rot_err},
            'rootMotion': root_motion,
            'npzSha256': sha(npz_bytes), 'npzBytes': len(npz_bytes),
            'metadataSha256': sha(meta_bytes),
            'boundary': 'Data export only. Death playback policy (sequence selection by hit direction, freeze at final pose, corpse retention) is a documented web port decision; the closed CS:GO client state machine is not reconstructed here.'}
        (out / 'death-audit.json').write_text(json.dumps(audit, indent=1) + '\n')
        summary[team] = {k: v for k, v in audit.items() if k not in ('descriptors', 'rootMotion')}
        print('DEATH_DECODED', team, json.dumps({'sequences': len(sequence_records),
            'frames': audit['frameDecoded'], 'samplerChecks': checks,
            'maxPositionError': frame_err, 'maxQuaternionDotError': rot_err,
            'npzSha256': audit['npzSha256']}))
        for rm in root_motion:
            print(f"  {rm['sequence']:26s} frames={rm['frames']:3d} fps={rm['fps']:5.1f} rootDelta={[round(v,3) for v in rm['rootDelta']]} frameStepMax={rm['maxFrameToFrameBoneMotion']:.4f}")
