async (page) => {
  ...
  await page.getByRole('button',{name:'Dust2 完整场景',exact:true}).click();
  ...
  await page.screenshot({path:'output/playwright/dust2-complete-ctspawn.png'});
  await page.getByRole('button',{name:'T 出生点',exact:true}).click();
  await page.waitForTimeout(800);
  await page.evaluate(()=>window.__CSGO_ASSET_PREVIEW__.report());
  const t=await page.evaluate(()=>window.__CSGO_ASSET_AUDIT__);
  await page.screenshot({path:'output/playwright/dust2-complete-tspawn.png'});
  if(ct.triangles!==6572252)throw Error('Full source world/prop geometry differs');
  if(errors.length)throw Error(errors.join('\n'));
  await page.evaluate(e=>window.__CSGO_MAP_EVIDENCE__=e,{ct,t,errors});
}
