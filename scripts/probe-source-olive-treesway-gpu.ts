import {SOURCE_OLIVE_TREESWAY_GLSL,SOURCE_OLIVE_TREESWAY_PROGRAM_SHA256} from '../game/source-olive-treesway-glsl';
...
export async function probeSourceOliveTreeswayGPU(){
  const response=await fetch('/assets/source-exports/dust2-foliage-audit/olive-power/treesway/oracle.json',{cache:'no-cache'});if(!response.ok)throw Error('Tree oracle HTTP '+response.status);
  const oracle=await response.json() as {cases:Case[];program:{programSha256:string}};
  if(oracle.program.programSha256!==SOURCE_OLIVE_TREESWAY_PROGRAM_SHA256||oracle.cases.length!==1280)throw Error('Original tree oracle differs');
...
void main(){resultPosition=sourceOlivePosition(position,timeWind,row0,row1,row2);gl_Position=vec4(0.0,0.0,0.0,1.0);}`));
    ...
    return {cases:oracle.cases.length,maxSourceError,maxPositionErrorMetres:maxSourceError*.0254,maxZeroWindError,maxMoved,
      worst:worst.sort((a,b)=>b.sourceError-a.sourceError).slice(0,5),programSha256:SOURCE_OLIVE_TREESWAY_PROGRAM_SHA256,glError:error,
