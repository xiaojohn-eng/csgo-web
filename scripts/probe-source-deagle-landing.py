...
p=load('pistol','probe-source-deagle-handling.py');l=load('landing','probe-source-landing.py')
...
def main():
 ...
 (ROOT/'output/tests/source-deagle-landing-native.json').write_text(json.dumps(report,separators=(',',':'))+'\n');print(json.dumps(dict(status=report['status'],rows=len(rows),chains=len(chains))))
