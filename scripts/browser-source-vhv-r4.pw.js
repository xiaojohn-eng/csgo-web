async(page)=>{
  ...
  const frames=[];
  for(const mode of ['r3','r4']){
    await page.evaluate(value=>window.__CSGO_ASSET_PREVIEW__.enablePropLighting(true,{enablePlainUnbumped:true,enableDecalMultiply:true,enableTintMask:value==='r4'}),mode);
    for(const team of ['ct','t']){
      ...
      const screenshot=`output/playwright/source-vhv-r4-${team}-${mode}.png`;
  ...
  const evidence={session:'csgo-vhv-r4',headless:true,url:page.url(),viewport:page.viewportSize(),
  ...
  await page.evaluate(value=>window.__CSGO_VHV_R4_EVIDENCE__=value,evidence);
  ...
  for(const team of ['ct','t']){
    ...
    const [r3,r4]=teamFrames;
    if(r3.state.lighting?.appliedMeshes!==2008||r4.state.lighting?.tintMaskMeshes<1||r4.state.lighting?.decalMultiplyMeshes!==40||r4.state.lighting?.appliedMeshes!==2008+r4.state.lighting?.tintMaskMeshes)
      throw Error('Unexpected VHV branch coverage');
    for(const next of teamFrames.slice(1))for(const key of ['calls','triangles'])if(r3.state.asset.render[key]!==next.state.asset.render[key])
