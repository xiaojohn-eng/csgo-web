...
inventory = json.loads((ROOT / '.reference-assets/source-exports/deagle-candidates/inventory.json').read_text())
...
rows, profiles = [], {}
for weapon, item, expected in [('deagle','1',{'damage':53,'headshot multiplier':3.9,'armor ratio':1.864,'range':4096,'range modifier':.85})]:
...
report = {'sourceServerSha256': native.SHA, 'sourceItems': source, 'profiles': profiles,
          'status': 'original_normal_deagle_damage_blocks_executed', 'rows': rows,
...
(ROOT / 'output/tests/source-deagle-damage-native.json').write_text(json.dumps(report, indent=2) + '\n')
