...
import {loadSourceM4A4Viewmodel} from '../game/source-m4a4-viewmodel';
...
async function fixture(profile:'m4'){
 const folder=resolve('public/source/csgo-12426148/'+'m4a4');
 ...
 return {buffers,gltf,loader:loadSourceM4A4Viewmodel};
}
describe.runIf(existsSync('public/source/csgo-12426148/m4a4/manifest.json'))('original first-person owners',()=>{
 it.each(['m4']as const)('%s validates original GLB and every PNG on HTTP without WebCrypto, and disposes only the requested clone',async profile=>{
  ...
  try{
   const owner=await f.loader({baseUrl:'/fixture'});expect(owner.hashVerified).toBe(true);expect(owner.profile).toBe('ct_arms_idf');
   expect(textures).toHaveLength(7);expect(urls.filter(u=>u.endsWith('.png'))).toHaveLength(textures.length);
 ...
 it.each(['m4']as const)('%s rejects a corrupt PNG and waits for all other loads before reclaiming assets',async profile=>{
  ...
  try{await expect(f.loader({baseUrl:'/fixture'})).rejects.toThrow(/SHA mismatch/);expect(textures).toHaveLength(6);expect(new Set(disposed)).toEqual(new Set(textures));expect(geometryDisposed).toBeGreaterThan(0);}
  finally{parser.mockRestore();texture.mockRestore();vi.unstubAllGlobals();}
 });
