...
import {loadSourceOlive,prepareSourceOliveData,type SourceOliveDescriptor} from '../game/source-olive-loader';
const descriptor=JSON.parse(readFileSync('.reference-assets/source-exports/dust2-vhv/olive-foliage/bindings.json','utf8')) as SourceOliveDescriptor;
...
it('is off by default and performs no fetch or scene work',async()=>{
 const fetch=vi.fn();vi.stubGlobal('fetch',fetch);try{const f=fixture(),owner=await loadSourceOlive(f.gltf,{csm:false,baseURL:'http://lan/vhv/olive-foliage/',state:{timeSeconds:0,windSourceXY:[0,0]}});expect(owner.audit.enabled).toBe(false);expect(fetch).not.toHaveBeenCalled();expect(f.mesh.material).toBe(f.material);}finally{vi.unstubAllGlobals();}
});
it('verifies exact original matrices/accessors and refuses changed bytes before material mutation',async()=>{
 const f=fixture();const result=await prepareSourceOliveData(f.gltf,f.d,f.remap,f.lighting);expect(result.bindings).toHaveLength(1);expect(result.verification.hashVerified).toBe(true);expect(f.mesh.material).toBe(f.material);
 f.mesh.matrix.elements[12]+=.1;await expect(prepareSourceOliveData(f.gltf,f.d,f.remap,f.lighting)).rejects.toThrow(/transform/);f.mesh.matrix.elements[12]-=.1;
 f.mesh.geometry.attributes.uv.setY(0,.1);await expect(prepareSourceOliveData(f.gltf,f.d,f.remap,f.lighting)).rejects.toThrow(/SHA/);expect(f.mesh.material).toBe(f.material);
});
it('enforces compact-byte identity and cancellation before binding',async()=>{
 const f=fixture(),bad=f.lighting.slice(0);new Uint8Array(bad)[0]^=1;await expect(prepareSourceOliveData(f.gltf,f.d,f.remap,bad)).rejects.toThrow(/receipt/);
 const abort=new AbortController();abort.abort();await expect(prepareSourceOliveData(f.gltf,f.d,f.remap,f.lighting,abort.signal)).rejects.toThrow();expect(f.mesh.material).toBe(f.material);
});
