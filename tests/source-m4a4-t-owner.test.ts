...
import {loadSourceM4A4TViewmodel} from '../game/source-m4a4-t-viewmodel';
import {inspectSourceViewmodel,isSourceViewmodel,sampleSourceViewmodel} from '../game/source-viewmodel';

async function fixture(profile:'m4'){
 const folder=resolve('public/source/csgo-12426148/'+'m4a4-t');
 ...
 return {buffers,gltf,loader:loadSourceM4A4TViewmodel};
}
describe.runIf(existsSync('public/source/csgo-12426148/m4a4-t/manifest.json'))('original first-person owners',()=>{
 it.each(['m4']as const)('%s validates original GLB and every PNG on HTTP without WebCrypto, and disposes only the requested clone',async profile=>{
  ...
  try{
   const owner=await f.loader({baseUrl:'/fixture'});expect(owner.hashVerified).toBe(true);expect(owner.profile).toBe('t_arms');
   expect(textures).toHaveLength(9);expect(urls.filter(u=>u.endsWith('.png'))).toHaveLength(textures.length);
   ...
   expect([...disposals.values()]).toEqual(textures.map(()=>0));sampleSourceViewmodel(b,'reload',.8);expect(inspectSourceViewmodel(b).time).toBe(.8);
 ...
 it.each(['m4']as const)('%s rejects a corrupt PNG and waits for all other loads before reclaiming assets',async profile=>{
  ...
  vi.stubGlobal('fetch',vi.fn(async(url:string)=>{const key=url.replace('/fixture/',''),data=f.buffers.get(key)!;
   const copy=Uint8Array.from(data);if(key==='textures/rif_m4a1_exponent-rgba.png')copy[copy.length-1]^=1;return new Response(new Blob([copy]));}));
  ...
  try{await expect(f.loader({baseUrl:'/fixture'})).rejects.toThrow(/SHA mismatch/);expect(textures).toHaveLength(8);expect(new Set(disposed)).toEqual(new Set(textures));expect(geometryDisposed).toBeGreaterThan(0);}
  finally{parser.mockRestore();texture.mockRestore();vi.unstubAllGlobals();}
 });
 it('aborts after texture decode and reclaims every resource already returned by loaders',async()=>{
  const f=await fixture('m4'),controller=new AbortController(),textures:T.Texture[]=[],disposed:T.Texture[]=[];
