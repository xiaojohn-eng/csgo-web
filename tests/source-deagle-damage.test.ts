...
const nativePath='output/tests/source-deagle-damage-native.json';
const original=fs.existsSync(nativePath)?it:it.skip;
original('matches the original Deagle normal damage blocks across armor and range boundaries',()=>{
 ...
 expect(oracle.rows).toHaveLength(630);let maxFloatError=0;
 ...
 fs.writeFileSync('output/tests/source-deagle-damage-validation.json',JSON.stringify({status:'passed',cases:oracle.rows.length,maxFloatError,integerMismatches:0,sourceServerSha256:oracle.sourceServerSha256},null,2)+'\n');
});
it('does not extend pistol bullet damage beyond the original 4096 unit trace',()=>{
 for(const weapon of ['deagle']as SourceBulletWeapon[]){
