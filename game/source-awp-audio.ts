...
import catalog from './source-awp-audio.json';
import {sourceSha256} from './source-sha256';

/** Explicit AWP sound owner. The caller awaits readiness before first draw,
 * dispatches original accepted shot events and owns animation audio cursors. */
export async function loadSourceAWPAudio(audio:AudioEngine,options:{signal?:AbortSignal}={}){
 const {signal}=options,records=catalog.weapons.awp.records;
 if(audio.disposed)throw Error('Audio engine disposed');
 signal?.throwIfAborted();
 if(typeof OfflineAudioContext==='undefined')throw Error('Original AWP audio decoder unavailable');
 ...
 try{
  const pending=await Promise.allSettled(records.map(async row=>{
   ...
   if(!buffer){buffer=(async()=>{
    const response=await fetch(row.url,{cache:'no-cache',signal});if(!response.ok)throw Error('AWP sound HTTP '+response.status);
    const bytes=new Uint8Array(await response.arrayBuffer());
    if(bytes.byteLength!==row.bytes||await sourceSha256(bytes,signal)!==row.sha256)throw Error('Original AWP sound SHA mismatch');
  ...
  return{hashVerified,records,timeline:catalog.weapons.awp.timeline,event,dispose};
