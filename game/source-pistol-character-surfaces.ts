...
/** World USP retains boost 5, white Phong, HalfLambert and its absent envmap. */
export function createSourceWorldPistolMaterial(weapon:'glock'|'usp',textures:{base:T.Texture;exponent:T.Texture}){
...
export function applySourcePistolCharacterSurfaces(gltf:GLTF,team:'t'|'ct',weapon:'glock'|'usp',textures:ReadonlyMap<string,T.Texture>){
 const handles=new Map<string,ReturnType<typeof createSourceCharacterMaterial>>(),bindings:{mesh:T.Mesh;original:T.Material|T.Material[];assigned:T.Material|T.Material[]}[]=[],seen=new Set<string>();let disposed=false;
