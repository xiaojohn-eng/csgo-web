...
import {applySourceOliveLighting,type SourceOliveBinding} from './source-olive-lighting';
import {sourcePropOliveCandidate,validateSourceTreeState,type SourceTreeState,type SourceTreeBinding} from './source-prop-olive';
import {SOURCE_OLIVE_TREESWAY_PROGRAM_SHA256} from './source-olive-treesway-glsl';
...
export interface SourceOliveDescriptor {format:'source-prop-olive-v1';sourceBspSha256:string;originalGLBSha256:string;programSha256:string;
...
export async function prepareSourceOliveData(gltf:GLTF,d:SourceOliveDescriptor,remap:ArrayBuffer,lighting:ArrayBuffer,signal?:AbortSignal){
 signal?.throwIfAborted();
 if(d.format!=='source-prop-olive-v1'||d.sourceBspSha256!==bsp||d.originalGLBSha256!==original||d.programSha256!==SOURCE_OLIVE_TREESWAY_PROGRAM_SHA256||!Array.isArray(d.records)||d.records.length>64||!Array.isArray(d.materials))throw Error('Original foliage descriptor identity differs');
 ...
 if(records.size!==d.records.length||materials.size!==d.materials.length||d.materials.some(m=>!sourcePropOliveCandidate(m)))throw Error('Duplicate/unsupported original foliage identity');
 const meshes:T.Mesh[]=[];gltf.scene.traverse(o=>{if((o as T.Mesh).isMesh)meshes.push(o as T.Mesh);});
 gltf.scene.updateWorldMatrix(true,true);const inverse=gltf.scene.matrixWorld.clone().invert(),bindings:SourceOliveBinding[]=[];
...
export async function loadSourceOlive(gltf:GLTF,options:{enabled?:boolean;csm:false;baseURL:string;state:SourceTreeState;signal?:AbortSignal;maxTextureSize?:number}){
 if(!options.enabled)return {audit:{enabled:false},dispose(){},setState(_state:SourceTreeState){},batch:undefined};
 if(options.csm!==false)throw Error('Original olive candidate requires explicit no-CSM branch');
 ...
 const manifest=await response.json() as {format:string;file:Receipt};if(manifest.format!=='source-prop-olive-receipt-v1'||manifest.file.url!=='bindings.json')throw Error('Original foliage manifest differs');
 ...
 const d=JSON.parse(new TextDecoder().decode(await bytes(manifest.file))) as SourceOliveDescriptor;
 if(d.records?.length!==64)throw Error('Original 64 olive bindings required');
 const [remap,lighting]=await Promise.all([bytes(d.files['remap.u32']),bytes(d.files['lighting.bin'])]);
 const data=await prepareSourceOliveData(gltf,d,remap,lighting,signal);signal?.throwIfAborted();
 const owner=applySourceOliveLighting(gltf.scene,{...data,csm:false,state:options.state,maxTextureSize:options.maxTextureSize});
 // Batch inputs are the exact verified bindings the shader owner installed;
 // the shared lookup texture and animated wind vector are borrowed, not owned.
 const batch={bindings:data.bindings,remap:data.remap,lookup:owner.lookup,timeWind:owner.timeWind};
 return {audit:{enabled:true,...owner.audit,verification:data.verification},dispose:owner.dispose,setState:owner.setState,batch};
}
